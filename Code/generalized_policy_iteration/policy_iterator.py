...
class PolicyIterator(DynamicProgrammingBase):
    ...
    def _improve_policy(self) -> bool:
        ...
        for x in range(map.width()):
            for y in range(map.height()):
                # Skip obstructions and terminal states since policy does not change there
                if map.cell(x, y).is_obstruction() or map.cell(x, y).is_terminal():
                    continue

                # Save the current action for this cell
                current_action = self._pi.action(x, y)

                best_action = None
                best_q = float('-inf')

                # Assume the environment provides a list of available actions for a cell
                for action in range(environment.available_actions().n):
                    # Compute the Q-value for taking this action from state (x,y)
                    s_prime, r, p = environment.next_state_and_reward_distribution((x, y), action)
                    q_value = 0
                    for t in range(len(p)):
                        # Get coordinates of the next state s'
                        s_prime_coords = s_prime[t].coords()
                        # Sum the contributions: probability * (reward + discounted future value)
        ...
        # Return true if the policy did not change (i.e., is stable)
